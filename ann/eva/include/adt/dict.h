...
extern struct dict_entry *dictAddOrFind(struct dict *d, const union value key,
                                        _mut_ int *existed);
extern error_t dictAdd(struct dict *d, const union value key, union value val);
extern int dictReplace(struct dict *d, const union value key, union value val);
...
#define dictSetKey(d, entry, _union_key_) _DICT_SET_KEY(d, entry, _union_key_)
...
#define _DICT_SET_DATA(d, entry, _val_)                                  \
        do {                                                             \
                ...
                if ((d)->type->valDupFn)                                 \
                        (entry)->v.data =                                \
                            (d)->type->valDupFn((d)->privdata, (_val_)); \
                else {                                                   \
                        (entry)->v.data = (_val_);                       \
                }                                                        \
                if (prev_data != NULL) {                                 \
