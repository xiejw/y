...
#define vecReserve(pvec, count) _VEC_RESERVE_IMPL(pvec, count)  // ret error_t
...
#define _VEC_CHECK_MAGIC(vec) (assert(((size_t *)(vec))[-3] == _VEC_MAGIC_NUM))
#else
...
#define _VEC_SET_SIZE_IMPL(vec, new_s)                                        \
        ((vec) ? (_VEC_CHECK_MAGIC((vec)), ((size_t *)vec)[-2] = (new_s), OK) \
               : ENOTEXIST)
...
#define _VEC_EXTEND_IMPL(pdst, src)                                       \
        _vecExtend((size_t **)(pdst), vecSize(*(pdst)), sizeof(**(pdst)), \
