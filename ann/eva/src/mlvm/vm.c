...
vmBatch( struct vm *vm, size_t size, const struct vm_oparg *args )
{
    ...
    for ( size_t i = 0; i < size; i++ ) {
        ...
        if ( err ) {
            return errEmitNote( "failed to exec %d-th instruction.", i );
        }
    }
...
vmOpsToSds( sds_t *psds, size_t size, const struct vm_oparg *args )
{
    ...
    for ( size_t i = 0; i < size; i++ ) {
        arg = &args[i];
        if ( arg->has_opt ) errFatalAndExit1( "has opt does not support yet" );
...
#define CASE_ELEWISE_OP( OP, API )                        \
    case VM_OP_##OP:                                      \
        ...
        return errNewWithNote(                            \
            ENOTIMPL, "unimpl forVM_OP_" #OP " with dtype %d", td->dtype );
...
#undef CASE_ELEWISE_OP
    ...
    case VM_OP_MATMUL:
        ...
        if ( td->dtype == VM_F32 ) {
            int trans_lhs = 0;
            int trans_rhs = 0;
            if ( opt != NULL && opt->mode != VM_OPT_MATMUL_TRANS_NOT ) {
                if ( opt->mode == VM_OPT_MATMUL_TRANS_LHS ) {
                    trans_lhs = 1;
                } else {
                    assert( opt->mode == VM_OPT_MATMUL_TRANS_RHS );
                    trans_rhs = 1;
                }
            }
        ...
        return errNewWithNote( ENOTIMPL, "unimpl forVM_OP_MATMUL with dtype %d",
                               td->dtype );
    ...
    case VM_OP_RNG:
        ...
        if ( td->dtype == VM_F32 ) {
            return vmOpRngF32( td, opt->mode & VM_OPT_MODE_UNMASK, &rng );
        }

        return errNewWithNote( ENOTIMPL, "unimpl forVM_OP_RNG with dtype %d",
                               td->dtype );
    ...
    case VM_OP_ARGMAX:
        ...
        return errNewWithNote( ENOTIMPL, "unimpl forVM_OP_ARGMAX with dtype %d",
                               td->dtype );
    ...
    case VM_OP_REDUCE:
        ...
        if ( td->dtype != VM_F32 ) {
            return errNewWithNote(
                ENOTIMPL, "unimpl forVM_OP_REDUCE with dtype %d", td->dtype );
        }
        ...
        return vmOpReduceF32( td, t1, opt->mode & VM_OPT_MODE_UNMASK, axis );
    ...
    case VM_OP_FILL:
        ...
        if ( td->dtype != VM_F32 ) {
            return errNewWithNote(
                ENOTIMPL, "unimpl forVM_OP_FILL with dtype %d", td->dtype );
        }
    ...
    case VM_OP_ISQRT:
        ...
        if ( td->dtype != VM_F32 ) {
            return errNewWithNote(
                ENOTIMPL, "unimpl forVM_OP_ISQRT with dtype %d", td->dtype );
        }
    ...
    case VM_OP_LS_SCEL:
        ...
        if ( td->dtype != VM_F32 ) {
            return errNewWithNote(
                ENOTIMPL, "unimpl forVM_OP_LS_SCEL with dtype %d", td->dtype );
        }
