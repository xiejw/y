...
fpMkdirAll( const char *path, mode_t mode )
{
    ...
    if ( r == OK ) {
        ...
        if ( S_ISREG( m ) ) {
            return errNew( "path is expected to be dir but got file: %s",
                           path );
        }
    ...
    if ( j > 0 ) {
        ...
        if ( err ) {
            err = errEmitNote( "failed to mkdir parent dir: %s", parent_p );
            free( parent_p );
            return err;
        }
...
fpCopy( const char *dst_path, const char *src_path, mode_t mode )
{
    ...
    while ( ( size = read( src, buffer, FP_FILE_BUF_SIZE ) ) > 0 ) {
        ...
        if ( real_size == -1 ) {
            ...
            err = errNew( "faied to write all bytes into file %s", dst_path );
            goto exit;
        }
    }
    ...
    if ( size == -1 ) {
        err =
            errNew( "faied to read file %s: %s", src_path, strerror( errno ) );
        goto exit;
    }
