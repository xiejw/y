             ...
             size_t init_num_slots )
{
    ...
    long   num_bits_to_shift;
    {
        ...
        slots_per_line =
            bitOpsNext2Power( suggested_slots_per_line, &num_bits_to_shift );
        if ( num_bits_to_shift == -1 ) {
            errPanic( "suggested_slots_per_line for stack is too large." );
            return NULL;
        }
    ...
    vec_t( void * ) lines = vecNew( );
    {
        ...
        // same for 2 lines.
        num_lines = ( init_num_slots + slots_per_line - 1 ) / slots_per_line;
