...
Sds::catVprintf( const char *fmt, va_list ap )
{
        ...
        // Path 1: Fast path first. Use the remaining area if possible for
        // speed.
        {
        ...
        /* Try with buffers two times bigger every time we fail to fit the
         * string in the current buffer size. */
        while ( 1 ) {
                buf[buflen - 2] = '\0';
                va_copy( cpy, ap );
                vsnprintf( buf, buflen, fmt, cpy );
