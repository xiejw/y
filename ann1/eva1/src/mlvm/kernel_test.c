...
test_ele_ops( void )
{
        ...
        enum vm_opcode ops[] = { VM_OP_ADD,    VM_OP_MUL, VM_OP_MINUS,
                                 VM_OP_DIVIDE, VM_OP_MAX, VM_OP_EQ,
                                 VM_OP_CMPL };
        ...
        for ( int i = 0; i < (int)( sizeof( ops ) / sizeof( enum vm_opcode ) );
              i++ ) {
...
test_matmul( void )
{
        ...
        for ( int i = 0;
              i < (int)( sizeof( opts ) / sizeof( struct vm_opopt * ) ); i++ ) {
...
test_ls_scel( void )
{
        ...
        // label
        COPY_DATA( vm, t1, 6,
                   ( (f32_t[]){ 0.2f, 0.3f, .5f, 0.5f, 0.4f, .1f } ) );
        // logit
        COPY_DATA( vm, t2, 6,
